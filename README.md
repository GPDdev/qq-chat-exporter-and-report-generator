...
这是一组用于从 QQ NT 聊天数据库导出群聊纯文字记录、统计年度数据，并生成年度报告网页海报的 Python 脚本。项目当前以「25级国豪学生群年度报告」为示例，统计区间为 `2025-07-27 00:00:00` 至 `2026-07-26 23:59:59`（北京时间，UTC+8）。
...
```text
.
├── generate_poster.py          # 根据 草稿.md 生成 index.html 年度报告页面
├── work/
│   └── export_group_text.py    # 从 QQ NT 数据库导出纯文字聊天记录和统计
├── 草稿.md                     # 年度报告数据草稿，供 generate_poster.py 读取
└── README.md                   # 项目说明
```

如果你只想把两个脚本发布到 GitHub，可以把 `work/export_group_text.py` 移到仓库根目录，或在 README 中保留当前路径说明。
...
- 群号：`808688505`。
- 统计时间：`2025-07-27 00:00:00` 至 `2026-07-26 23:59:59`，北京时间（UTC+8）。
- 数据源：`nt_msg_plain.db` 中的 `group_msg_table`。
- 文字导出：仅导出能够明确识别的 `TextElement` 正文；原生 emoji 会作为文字保留。
- 非文字消息：图片、视频、商城表情、收藏表情及其他类型不会写入月度纯文字记录，但会参与消息类别统计。
...
- 发布到 GitHub 前，建议不要上传原始数据库、聊天记录、群成员 QQ 号、头像等隐私数据。

## GitHub 发布建议

建议仓库只保留脚本、示例草稿和脱敏后的示例输出：

```text
.
├── export_group_text.py
├── generate_poster.py
├── examples/
│   └── 草稿.example.md
├── .gitignore
...
本项目仅用于个人数据整理与可视化展示。请在导出、分析和发布聊天记录前获得相关成员同意，并对涉及个人身份、账号、头像、聊天内容的数据进行脱敏处理。
